...
def tokenize_function(examples):
    tokenized_text = tokenizer(examples["context"], padding="max_length", truncation=True, max_length=128)
...
training_args_combined = TrainingArguments(
    ...
    dataloader_num_workers=1,
    learning_rate=1e-5,  # Lower learning rate for more stable combined training
...
if __name__ == "__main__":
    ...
    for name, model in trained_models.items():
        ...
        responses = evaluate_model_output(model, evaluation_questions, tokenizer)
        
        # Save the generated responses to a file
        with open(f"{name}_responses.txt", "w", encoding="utf-8") as file:
            for question, response in zip(evaluation_questions, responses):
                file.write(f"Question: {question}\nResponse: {response}\n\n")

    print("\nGenerated responses saved to files for APL, DPO, and Combined models.")
