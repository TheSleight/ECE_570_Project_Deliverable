...
def evaluate_model_output(model, eval_dataset, tokenizer):
    model.eval()
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            outputs = model.generate(input_ids, max_length=150, num_return_sequences=1, repetition_penalty=1.2)  # Add repetition penalty to discourage repeated phrases
        ...
        # Calculate ROUGE score for each response
        for ref, res in zip(references, responses):
...
if __name__ == "__main__":
    ...
    for name, model in trained_models.items():
        ...
        bleu_scores, rouge_scores = evaluate_model_output(model, eval_dataset, tokenizer)
        
        # Save the evaluation metrics to a file
        with open(f"{name}_evaluation_metrics.txt", "w", encoding="utf-8") as file:
            file.write(f"{name} Model Evaluation Metrics\n")
            file.write(f"BLEU Scores: {bleu_scores}\n")
            file.write(f"ROUGE Scores: {rouge_scores}\n")

    print("\nEvaluation metrics saved for APL, DPO, and Combined models.")
